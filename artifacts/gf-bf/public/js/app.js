(() => {
  ...
  function renderLogin() {
    return `<main class="screen login-hero">
      ...
      <div class="stack mt-6">
        <button class="btn btn-ghost btn-block" data-act="guest">${t("guest")}</button>
        <div class="muted center" style="font-size:12px">— ${getLang() === "bn" ? "অথবা ইমেইল" : "or email"} —</div>
  ...
  root.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (act === "guest") {
        const res = await API.guest({ displayName: "Guest" });
        API.setToken(res.token);
        state.user = res.user;
        state.page = "home";
        await loadRooms();
