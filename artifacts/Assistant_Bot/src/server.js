...
app.get("/api/config", (_req, res) => {
  res.json({
    ...
    botName: process.env.BOT_NAME || "Assistant Bangladesh",
...
app.post("/api/heartbeat", auth, async (req, res) => {
  try {
    ...
    const online = await Presence.countDocuments({ lastSeenAt: { $gte: since } });
    // Live social proof: fluctuate between ~30,000 – 40,000
    const base = 30000 + (online % 5000);
    const jitter = Math.floor(Math.random() * 5000);
    const displayOnline = Math.min(40000, Math.max(30000, base + jitter));
    res.json({ online: displayOnline, real: online });
  } catch (e) {
    // fallback still in 30k–40k range
    res.json({ online: 30000 + Math.floor(Math.random() * 10000) });
  }
});
...
app.post("/api/topup", auth, async (req, res) => {
  try {
    ...
    // In production top-up would be payment gateway; here we create pending record
    await Transaction.create({
      telegramId: user.telegramId,
      type: "topup",
      amount,
    ...
    res.json({ ok: true, message: "Top-up request submitted. Admin will process." });
  } catch (e) {
