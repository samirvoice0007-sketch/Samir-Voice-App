...
function getBot() {
  ...
  bot.command("help", async (ctx) => {
    ...
    await ctx.reply(
      `🛠 Owner Command List\n\n` +
        ...
        `/topups — List pending top-up requests\n` +
        `/approvetopup <id> — Credit USDT + run referral commission\n` +
        `/rejecttopup <id> — Reject a top-up request\n\n` +
  ...
  async function distributeCommission(fromUser, amount) {
    ...
    for (let level = 0; level < COMMISSION_RATES.length && currentId; level++) {
      ...
      await Transaction.create({
        ...
        meta: { from: fromUser.telegramId, level: level + 1 }
      });
  ...
  bot.command("topups", async (ctx) => {
    if (!(await isAdmin(ctx.from.id))) return;
    const list = await Transaction.find({ type: "topup", status: "pending" })
      .sort({ createdAt: -1 })
      .limit(20);
    if (!list.length) return ctx.reply("No pending top-ups.");
    const lines = list.map(
      (t) => `ID: \`${t._id}\`\nUser: ${t.telegramId}\nAmount: $${t.amount} USDT\n---`
    );
    await ctx.reply(lines.join("\n"), { parse_mode: "Markdown" });
  ...
  bot.command("approvetopup", async (ctx) => {
    if (!(await isAdmin(ctx.from.id))) return;
    const id = (ctx.message.text.split(/\s+/)[1] || "").trim();
    if (!id) return ctx.reply("Usage: /approvetopup <id>");
    const tx = await Transaction.findById(id);
    if (!tx || tx.type !== "topup" || tx.status !== "pending") {
      return ctx.reply("Invalid or already processed.");
    }
    const user = await User.findOne({ telegramId: String(tx.telegramId) });
    if (!user) return ctx.reply("User not found.");

    const amount = Number(tx.amount) || 0;
    user.coins = (user.coins || 0) + amount;
    const total = (user.coins || 0) + (user.usdt || 0);
    const thresholds = [0, 3, 10, 30, 100, 300, 1000];
    let lvl = 1;
    for (let i = 0; i < thresholds.length; i++) {
      if (total >= thresholds[i]) lvl = i + 1;
    }
    user.level = Math.min(7, lvl);
    await user.save();

    tx.status = "completed";
    await tx.save();

    try { await distributeCommission(user, amount); } catch (e) { console.error("commission", e); }

    try {
      await bot.telegram.sendMessage(
        user.telegramId,
        `✅ Your top-up of $${amount} USDT has been approved.\nBalance updated. Start earning!`
      );
    } catch (_) {}

    await ctx.reply(`✅ Top-up ${id} approved. Credited $${amount} coins to user ${user.telegramId}.`);
  });

  bot.command("rejecttopup", async (ctx) => {
    if (!(await isAdmin(ctx.from.id))) return;
    const id = (ctx.message.text.split(/\s+/)[1] || "").trim();
    if (!id) return ctx.reply("Usage: /rejecttopup <id>");
    const tx = await Transaction.findById(id);
    if (!tx || tx.type !== "topup" || tx.status !== "pending") {
      return ctx.reply("Invalid or already processed.");
    }
    ...
    try {
      await bot.telegram.sendMessage(
        tx.telegramId,
        `❌ Your top-up of $${tx.amount} USDT was rejected. Contact support if needed.`
      );
    } catch (_) {}
